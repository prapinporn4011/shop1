...
const products = dbProducts.map(p => ({
        ...
        container.innerHTML = items.map(p => `
            <div class="col-6 col-md-3 mb-4">
                <div class="card product-card shadow-sm border-0">
                    ...
                    <img src="${p.img}" class="card-img-top" alt="${p.name}" style="cursor:pointer;" onclick="viewDetail(${p.id})" onerror="this.onerror=null; this.src='https://placehold.co/300x400/1a1a1a/ffae00?text=No+Image'">
                    <div class="card-body">
                        ...
                        <div class="mt-3">
                            ...
                            <button class="btn btn-dark btn-sm w-100 mt-2 fw-bold" onclick="viewDetail(${p.id})">ดูรายละเอียด / สั่งซื้อ</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    // --------------------------------------------------------
    // ระบบตะกร้า และ รายละเอียดสินค้า
    // --------------------------------------------------------
    function viewDetail(id) {
        if(!currentUser) {
            openAuthModal('login');
            return showToast('กรุณาเข้าสู่ระบบก่อนดูรายละเอียดหรือสั่งซื้อ', 'warning');
        }
        ...
        document.getElementById('detail-name').innerText = p.name;
        document.getElementById('detail-price').innerText = '฿' + p.price.toLocaleString();
        document.getElementById('detail-desc').innerText = p.desc || 'ไม่มีรายละเอียด';
        document.getElementById('detail-type').innerText = p.type;
        
        const imgEl = document.getElementById('detail-img');
        imgEl.src = p.img;
        imgEl.onerror = function() { this.src = 'https://placehold.co/300x400/1a1a1a/ffae00?text=No+Image'; };
    ...
    function addToCart(isBuyNow) {
        ...
        if(isBuyNow) setTimeout(openCart, 400);
        else showToast('เพิ่มลงตะกร้าเรียบร้อย', 'success');
    }
    ...
    function openCart() {
        if(!currentUser) return showToast('กรุณาเข้าสู่ระบบก่อนเปิดตะกร้า', 'warning');
        
        const container = document.getElementById('cart-items-container');
        if(cart.length === 0) {
            container.innerHTML = '<p class="text-center text-muted my-5">ไม่มีสินค้าในตะกร้า</p>';
            ...
            container.innerHTML = cart.map((item, index) => {
                ...
                return `
                    <div class="d-flex align-items-center mb-3 p-2 border rounded bg-white">
                        <img src="${item.img}" width="60" height="60" class="rounded me-3 border" style="object-fit: cover;" onerror="this.src='https://placehold.co/60x60'">
                        <div class="flex-grow-1">
                            <h6 class="mb-0 fw-bold">${item.name}</h6>
                            <small class="text-muted">ไซส์: <strong>${item.size}</strong> | จำนวน: ${item.qty}</small><br>
                            <small class="text-danger">฿${item.price.toLocaleString()} / ชิ้น</small>
                        </div>
                        <div class="fw-bold me-3">฿${total.toLocaleString()}</div>
                        <button class="btn btn-sm text-danger" onclick="removeCartItem(${index})"><i class="fa fa-trash"></i></button>
                    </div>
                `;
            }).join('');
            document.getElementById('summary-subtotal').innerText = '฿' + subtotal.toLocaleString();
        ...
        // ดึงข้อมูลที่อยู่จากโปรไฟล์มาใส่เป็นค่าเริ่มต้น
        document.getElementById('ship-name').value = currentUser.name || '';
        document.getElementById('ship-phone').value = currentUser.phone || '';
    ...
    // --------------------------------------------------------
    // ระบบชำระเงิน & ยิง API เข้า Database (save_order.php)
    // --------------------------------------------------------
    function selectPayment(method) {
        paymentMethod = method;
    ...
    function collectCoupon(code) {
        discount = 50;
        updateTotal();
        showToast('ใช้คูปองส่วนลด 50 บาทเรียบร้อยแล้ว!', 'success');
    }
    ...
    function applyCoupon() {
        ...
        if(val === 'NEWBIE50') {
            ...
            showToast('คูปองไม่ถูกต้อง', 'error');
        }
    }
    ...
    function updateTotal() {
        ...
        document.getElementById('summary-total').innerText = '฿' + Math.max(0, subtotal + 50 - discount).toLocaleString();
    }
    ...
    function confirmOrder() {
        ...
        if(!shipName || !shipPhone || !shipAddress) {
            return showToast('กรุณากรอกข้อมูลการจัดส่งให้ครบถ้วน', 'warning');
        }

        bootstrap.Modal.getInstance(document.getElementById('checkoutModal')).hide();
        
        if(paymentMethod === 'QR') {
            // ถ้าเลือก QR ให้เปิดหน้าสแกนจ่ายก่อน
            ...
            qrTimerInterval = setInterval(() => {
                ...
                if(time <= 0) { clearInterval(qrTimerInterval); qrPaymentCancel(); }
            }, 1000);
        } else {
            // ถ้า COD ให้เซฟลง DB เลย
            processCheckout('COD', shipName, shipPhone, shipAddress);
        }
    }
    ...
    function qrPaymentSuccess() {
        ...
        processCheckout('QR Code', shipName, shipPhone, shipAddress);
    }
    ...
    // ++ ฟังก์ชันสำคัญ: บันทึกออเดอร์ลง Database ผ่าน PHP ++
    function processCheckout(method, name, phone, address) {
        const subtotal = cart.reduce((sum, i) => sum + (i.price * i.qty), 0);
        const total = Math.max(0, subtotal + 50 - discount);
        const isQR = (method === 'QR Code');
        const dbStatus = isQR ? 'pending' : 'paid'; // ส่งสถานะไปให้ PHP
        
        const fullAddress = `ชื่อ: ${name} โทร: ${phone}\nที่อยู่: ${address}`;

        // เตรียมข้อมูลส่งไปหา save_order.php
        const orderData = {
        ...
        .then(data => {
            if(data.success) {
                // เซฟจำลองในเครื่องเผื่อไว้โชว์หน้าประวัติชั่วคราว
                const newOrder = {
                    orderId: "TJ-" + data.order_id, // ใช้ ID จาก DB
                    items: [...cart],
                    method: method,
                    status: isQR ? 'รอตรวจสอบยอดเงิน' : 'เตรียมจัดส่ง',
                    total: total,
                    address: fullAddress,
                ...
                if(isQR) {
                    showToast('สั่งซื้อสำเร็จ! รอแอดมินตรวจสอบยอดเงิน', 'success');
                } else {
                    showToast('สั่งซื้อสำเร็จ! รอรับสินค้าที่บ้านได้เลย', 'success');
                }
                
                setTimeout(openOrderHistory, 600);
            } else {
                showToast('เกิดข้อผิดพลาดในการบันทึกข้อมูล กรุณาลองใหม่', 'error');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            showToast('เชื่อมต่อฐานข้อมูลล้มเหลว กรุณาตรวจสอบ db.php', 'error');
        });
    }
    ...
    function openOrderHistory() {
        ...
        if(orders.length === 0) {
            ...
            container.innerHTML = orders.map(o => {
                const isWarning = o.status.includes('รอ');
                return `
                <div class="bg-white p-3 rounded shadow-sm mb-3 border">
                    <div class="d-flex justify-content-between border-bottom pb-2 mb-2">
                        <span class="fw-bold text-dark">ออเดอร์: ${o.orderId}</span>
                        <span class="badge ${isWarning ? 'bg-warning text-dark' : 'bg-success'}">${o.status}</span>
                    </div>
                    ${o.items.map(item => `
                        <div class="d-flex justify-content-between align-items-center small mb-2 border-bottom pb-2">
                            <div class="d-flex align-items-center">
                                <img src="${item.img}" width="40" height="40" style="object-fit:cover;" class="rounded me-2">
                                <span>${item.name} <span class="text-muted">(Size: ${item.size})</span></span>
                            </div>
                            <span>x ${item.qty} = <strong>฿${(item.price * item.qty).toLocaleString()}</strong></span>
                        </div>
                    ...
                    <div class="d-flex justify-content-between align-items-end mt-3">
                        <small class="text-muted" style="max-width: 60%;">
                            <i class="fa fa-truck me-1"></i> ${o.method} <br>
                            <i class="fa fa-map-marker-alt me-1"></i> ${o.address}
                        </small>
                        <div class="text-end fw-bold">ยอดสุทธิ: <span class="text-danger fs-5 ms-2">฿${o.total.toLocaleString()}</span></div>
                    </div>
                </div>
